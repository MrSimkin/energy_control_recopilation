...
public sealed class InstallationHealthRepository
{
        ...
        IReadOnlyCollection<string> attributeKeys)
    {
        ...
        {
            return new Dictionary<string, RawInstallationValue>(StringComparer.Ordinal);
        }

        using var connection = _database.OpenConnection();
        using var command = connection.CreateCommand();

        var parameters = attributeKeys
        ...
        command.CommandText = $"""
            WITH ranked AS (
                ...
                WHERE device_id = $deviceId
                  AND is_missing = 0
                  AND attribute_key IN ({string.Join(",", parameters)})
            )
            SELECT
                attribute_key,
                recorded_at_utc,
        ...
        {
            command.Parameters.AddWithValue($"$key{index}", key);
            index++;
        }
        ...
        while (reader.Read())
        {
            ...
            result[key] = new RawInstallationValue(
                key,
                recordedAt,
